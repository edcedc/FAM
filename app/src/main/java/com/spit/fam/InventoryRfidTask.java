...
public class InventoryRfidTask extends AsyncTask<Void, String, String> {
    ...
    protected void onPreExecute() {
        ...
        if (button != null) button.setText(MainActivity.mContext.getString(R.string.stop).toUpperCase());
        ...
        if (output[0] != null) {
            ...
            long currentTime = 0;
            {
                while (rx000pkgDataArrary.size() != 0) {
                    ...
                    if (tagsList == null) {
                        if (strEpc != null && strEpc.matches(strEpcOld)) {
                            match = true;
                            updated = true;
                        }
                    } else if (readerListAdapter != null && readerListAdapter.getSelectDupElim()) {
...
//                                    saveExternalTask.openServer();
            ...
            //Log.i("tagList", "tagList readerListAdapter " + readerListAdapter);

            if (readerListAdapter != null) {
                //Log.i("tagList", "tagList readerListAdapter " + tagsList.size() + " " + readerListAdapter.getCount());
                Set<String> epcList = new HashSet<>();

                if(tagsList != null) {
                    for (int i = 0; i < tagsList.size(); i++) {
                        //Log.i("tagList", "tagList " + tagsList.get(i).getAddress() + " " + tagsList.get(i).getTid());
                        epcList.add(tagsList.get(i).getAddress());
                    }
                    ArrayList<String> listFromSet = new ArrayList<String>(epcList);
    ...
    public InventoryRfidTask(Context context, int extra1Bank, int extra2Bank, int data1_count, int data2_count, int data1_offset, int data2_offset,
        ...
        if(readerListAdapter != null)
            ...
            this.readerListAdapter = new ReaderListAdapter(context, R.layout.readers_list_item, MainActivity.sharedObjects.barsList, true, false);
    ...
    void DeviceConnectTask4InventoryEnding(TaskCancelRReason taskCancelRReason) {
        ...
        if (button != null) button.setText(MainActivity.mContext.getResources().getString(R.string.start).toUpperCase()); MainActivity.sharedObjects.runningInventoryRfidTask = false;
