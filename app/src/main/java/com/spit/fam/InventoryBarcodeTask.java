...
public class InventoryBarcodeTask extends AsyncTask<Void, String, String> {
    ...
    protected void onPreExecute() {
        ...
        try {
            button.setText("Stop");
            if (button1 != null) button1.setText("Stop");
        } catch (Exception e) {
...
//            readerListAdapter.notifyDataSetChanged();
    ...
    protected void onProgressUpdate(String... output) {
        if (output != null) {
            ...
            Log.i("barcode", "barcode " + output);
            if(output != null) {
                String barcodeScanned = "";
                for (int i = 0; i < output.length; i++) {
                    Log.i("bugfix", "bugfix " + output[i]);
                    if(output[i] != null && output[i].length() > 0) {
                        barcodeScanned = output[i];
                        EventBus.getDefault().post(new BarcodeScanEvent(barcodeScanned));
                        break;
                    }
                }
            }
    ...
    public InventoryBarcodeTask(ArrayList<ReaderDevice> tagsList, ReaderListAdapter readerListAdapter, EditText registerBarValue,
        ...
        this.button = button;
        try {
            textButton = button == null ? "" : button.getText().toString();
            this.button1 = button1;
            if (button1 != null) btextButton1 = button1.getText().toString();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    void DeviceConnectTask4InventoryEnding(TaskCancelRReason taskCancelRReason) {
        ...
        if (taskCancelReason == null) {
            ...
            try {
                button.setText(textButton);
                if (button1 != null) button1.setText(btextButton1);} catch (Exception e){
